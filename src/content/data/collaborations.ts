...
export interface Collaboration {
  ...
  stack?: string[];
  /** Enlace externo (repo/demo) si es público. Ausente ⟹ no se renderiza enlace. */
  url?: string;
...
export const COLLABORATIONS: readonly Collaboration[] = [
  {
    // Club del Barril — plataforma de fidelización (PRIVADA) para Cervecería
    // Santa Fe. Sebas participó en la construcción de la aplicación COMPLETA, no
    // solo el backend: API (NestJS), panel de administración (backoffice) y app
    // de socios. El proyecto es privado/comercial: no hay repo público, así que
    // NO se publica URL y la UI muestra la etiqueta "Repositorio privado"
    // (Req 7.1/7.4). Stack real verificado en los `package.json` del proyecto.
    name: "Club del Barril",
    description: {
      es: "Plataforma de fidelización para Cervecería Santa Fe. Participé en la construcción de la aplicación completa: la API backend (NestJS), el panel de administración y la app de socios. Sistema de puntos, canjes por QR, promociones, distribuidores y campañas de marketing por email/push.",
      en: "Loyalty platform for Cervecería Santa Fe. I took part in building the whole application: the NestJS backend API, the admin dashboard and the members app. Points system, QR-based redemptions, promotions, distributors and email/push marketing campaigns.",
    },
    role: {
